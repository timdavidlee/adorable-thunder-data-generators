...
def generate_addresses(n_samples: int, country_code: str | None = None) -> pd.DataFrame:
    ...
    street_names = np.random.choice(_STREET_NAMES, size=n_samples, replace=True)
    street_addresses = [
        f"{num} {name}" for num, name in zip(street_numbers, street_names)
    ]

    df = pd.DataFrame(
        sampled, columns=["city", "state_province", "country_code", "postal_code"]
    )
