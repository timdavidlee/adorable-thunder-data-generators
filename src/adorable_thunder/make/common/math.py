...
def round_weights_and_rebalance(weights: np.ndarray, precision: int = 4) -> np.ndarray:
    ...
    Args:
        ...
        precision: Decimal places for rounding. Higher values yield more precision but may cause
            the sum to drift from 1 due to rounding errors.
    """
...
def get_weights(num_items: int, power_scale: float = 2.0, precision: int = 4) -> np.ndarray:
    """
    Args:
        num_items: The number of items to generate weights for
        power_scale: The power to which the rank of the item is raised. Higher values will result in
...
def generate_weighted_enum_choices(
    ...
    weights = get_weights(num_items=len(items), power_scale=power_scale, precision=precision)
    return generate_weighted_random_choice(items, n_samples=num_samples, weights=weights)
